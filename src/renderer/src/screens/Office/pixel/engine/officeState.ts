...
import { takeOverCoworker, updateNpcs } from './npcManager';
import {
  pickStatusOrMumble,
  pickThinkingStatus,
  STATUS_DURATION_MAX,
...
export class OfficeState {
  ...
  private _characterCache: Character[] | null = null;
  ...
  addAgent(
    ...
    // Try to take over an NPC coworker instead of creating a new character
    const takeover = takeOverCoworker(this.characters);

    let palette: number;
    let hueShift: number;
    const maxPalette = getLoadedCharacterCount();
    if (takeover) {
      // Use the coworker's palette and seat
      palette =
        preferredPalette !== undefined && preferredPalette < maxPalette
          ? preferredPalette
          : takeover.palette;
      hueShift = preferredHueShift ?? takeover.hueShift;
    } else if (preferredPalette !== undefined && preferredPalette < maxPalette) {
    ...
    // Try preferred seat, then takeover seat, then any free seat
    ...
    if (!seatId && takeover?.seatId) {
      seatId = takeover.seatId;
    }
    if (!seatId) {
  ...
  setAgentActive(id: number, active: boolean): void {
    ...
    if (ch) {
      ch.isActive = active;
      if (active) {
        // Show thinking status when agent becomes active
        if (!ch.workStatus) {
          ch.workStatus = pickThinkingStatus();
          ch.workStatusTimer =
            STATUS_DURATION_MIN + Math.random() * (STATUS_DURATION_MAX - STATUS_DURATION_MIN);
        }
      } else {
        // Clear status when agent goes idle
        ch.workStatus = null;
        ch.workStatusTimer = 0;
      }
      if (!active) {
  ...
  setAgentTool(id: number, tool: string | null): void {
    ...
    if (ch) {
      ch.currentTool = tool;
      // Set work status with personality mumbles mixed in
      if (tool) {
        ch.workStatus = pickStatusOrMumble(tool, ch.palette);
        ch.workStatusTimer =
          STATUS_DURATION_MIN + Math.random() * (STATUS_DURATION_MAX - STATUS_DURATION_MIN);
      }
    }
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      // Tick work status timer for real agents (NPCs handled in npcManager)
      if (ch.id >= 0 && ch.workStatusTimer != null && ch.workStatusTimer > 0) {
        ch.workStatusTimer -= dt;
        if (ch.workStatusTimer <= 0 && ch.isActive) {
          ch.workStatus = pickStatusOrMumble(ch.currentTool, ch.palette);
          ch.workStatusTimer =
            STATUS_DURATION_MIN + Math.random() * (STATUS_DURATION_MAX - STATUS_DURATION_MIN);
        }
      }
    ...
    // Update NPCs (coworker team, 996 overtime)
    updateNpcs(
      dt,
      this.characters,
      this.walkableTiles,
  ...
  getCharacters(): Character[] {
    if (!this._characterCache) {
      this._characterCache = Array.from(this.characters.values());
    }
    return this._characterCache;
  }
