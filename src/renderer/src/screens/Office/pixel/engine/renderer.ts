import type { ColorValue } from '../ui-types';
import {
  ...
  STATUS_BG_PADDING_V,
  STATUS_BORDER_COLOR,
  STATUS_FONT_SIZE_DEFAULT,
  STATUS_MIN_ZOOM,
  STATUS_TEXT_COLOR,
  STATUS_VERTICAL_OFFSET_PX,
...
// ── Work status text above characters ───────────────────────────
...
function renderWorkStatus(
  ...
  if (zoom < STATUS_MIN_ZOOM) return;

  const fs = fontSize ?? STATUS_FONT_SIZE_DEFAULT;
  ctx.save();
  ctx.font = `${fs}px "FS Pixel Sans", monospace`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'bottom';

  // Track rendered label rects to avoid overlap
  const rendered: Array<{ x: number; y: number; w: number; h: number }> = [];

  for (const ch of characters) {
    if (!ch.workStatus) continue;
    if (ch.matrixEffect) continue;

    const sittingOff = ch.state === CharacterState.TYPE ? CHARACTER_SITTING_OFFSET_PX : 0;
    const textX = offsetX + ch.x * zoom;
    let textY = offsetY + (ch.y + sittingOff - STATUS_VERTICAL_OFFSET_PX) * zoom;

    const measured = ctx.measureText(ch.workStatus);
    const textW = measured.width;
    const bgW = textW + STATUS_BG_PADDING_H * 2;
    const bgH = fs + STATUS_BG_PADDING_V * 2;

    // Nudge up if overlapping with an existing label
    const bgX = textX - textW / 2 - STATUS_BG_PADDING_H;
    let bgY = textY - fs - STATUS_BG_PADDING_V;
    for (let attempt = 0; attempt < 3; attempt++) {
      let overlaps = false;
      for (const r of rendered) {
        if (bgX < r.x + r.w && bgX + bgW > r.x && bgY < r.y + r.h && bgY + bgH > r.y) {
          overlaps = true;
          break;
        }
      }
      if (!overlaps) break;
      // Nudge up by one label height
      textY -= bgH + 2;
      bgY = textY - fs - STATUS_BG_PADDING_V;
    }

    rendered.push({ x: bgX, y: bgY, w: bgW, h: bgH });

    // Background panel
    ctx.fillStyle = STATUS_BG_COLOR;
    ctx.fillRect(bgX, bgY, bgW, bgH);

    // Border
    ctx.strokeStyle = STATUS_BORDER_COLOR;
    ctx.lineWidth = 1;
    ctx.strokeRect(bgX + 0.5, bgY + 0.5, bgW - 1, bgH - 1);

    // Text
    ctx.fillStyle = STATUS_TEXT_COLOR;
    ctx.fillText(ch.workStatus, textX, textY);
  }
...
function getDayNightOverlay(): string {
...
function renderDayNightOverlay(
  ...
  if (color === DAY_NIGHT_TRANSPARENT) return;
  ctx.save();
  ctx.fillStyle = color;
  const s = TILE_SIZE * zoom;
...
export function renderFrame(
  ...
  // Work status text above characters
  renderWorkStatus(ctx, characters, offsetX, offsetY, zoom, statusFontSize);

  // Day/night cycle tint overlay (on top of scene, below editor UI)
  renderDayNightOverlay(ctx, offsetX, offsetY, zoom, cols, rows);
