...
const pool = new AgentPool(configStore, {
  output: (payload) => { const task = taskQueue.findRunningByAgent(payload.agentId); if (task) repository.addEvent(task.taskId, "taskOutput", { stream: payload.stream, text: payload.line }); void server.sendNotification("agent/output", payload); },
  ...
  exit: (payload) => { const task = taskQueue.findRunningByAgent(payload.agentId); if (task) { try { const finished = taskQueue.finish(task.taskId, payload.killed ? "cancelled" : payload.code === 0 ? "succeeded" : "failed"); recordTask(finished, finished.status === "succeeded" ? "taskSucceeded" : finished.status === "cancelled" ? "taskCancelled" : "taskFailed"); repository.finishAttempt(task.taskId, payload.runId, finished.status, payload.code); } catch { /* task may already be terminal */ } } starOffice.publish(payload.agentId, payload.code === 0 ? "idle" : "error", payload.code === 0 ? "任务已完成，待命中" : "任务执行出现问题"); void server.sendNotification("agent/exit", payload); },
  error: (payload) => { const task = taskQueue.findRunningByAgent(payload.agentId); if (task) { try { const failed = taskQueue.finish(task.taskId, "failed"); recordTask(failed, "taskFailed", { message: payload.message }); repository.finishAttempt(task.taskId, task.runId ?? 0, "failed", null, payload.message); } catch { /* task may already be terminal */ } } starOffice.publish(payload.agentId, "error", "Agent 运行出现问题"); void server.sendNotification("agent/error", payload); },
  log: (payload) => { void server.sendNotification("sidecar/log", payload); },
});
const handlers: RpcHandlers = { ping: () => ({ ok: true, version: VERSION }), configReload: () => pool.reloadConfig(), startAgent: (params) => pool.startAgent(params), stopAgent: (params) => pool.stopAgent(params), listAgents: () => pool.listAgents(VERSION), taskSubmit: (params) => { const result = taskQueue.submit(params.proposal); if (result.accepted && result.envelope) { repository.saveTask({ taskId: result.envelope.taskId, proposalId: result.envelope.proposalId, title: result.envelope.title, description: result.envelope.description, assignedWorkerId: result.envelope.assignedWorkerId, status: "admitted", writeScope: result.envelope.writeScope, createdAt: result.envelope.createdAt, updatedAt: new Date().toISOString() }); repository.addEvent(result.envelope.taskId, "taskCreated", { title: result.envelope.title }); repository.addEvent(result.envelope.taskId, "taskAdmitted", { workerId: result.envelope.assignedWorkerId }); const queued = taskQueue.queue(result.envelope.taskId); recordTask(queued, "taskQueued"); startQueuedTask(result.envelope.taskId); return { ...result, envelope: { ...result.envelope, status: "queued" } }; } return result; }, taskList: () => ({ tasks: taskQueue.list() }), taskEvents: (params) => ({ events: repository.listEvents(params.taskId, params.limit) }), taskCancel: (params) => { const task = taskQueue.get(params.taskId); if (task?.status === "running" && task.runId !== undefined) { void pool.stopAgent({ agentId: task.assignedWorkerId, runId: task.runId }); } const cancelled = taskQueue.cancel(params.taskId); recordTask(cancelled, "taskCancelled"); return { task: cancelled }; }, shutdown: () => ({ ok: true }) };
