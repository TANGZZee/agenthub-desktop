...
export function restoreTasks(repository: TaskRepository): TaskRecord[] { return repository.listTasks().map((task) => ({ ...task, cwd: process.cwd(), proposedWorkerId: task.assignedWorkerId, allowedPaths: [], forbidden: [], toolPolicy: [], dependsOn: [], status: task.status as TaskRecord["status"] } as TaskRecord)); }
