...
## Off-screen tabs must stop polling

Tabs lazy-mount on first visit and then stay mounted behind `display: none`, so a screen's timers keep running after the user has navigated away.

A polling screen that ignores this keeps doing real work forever — and the cost is not just CPU: any poll that reaches the main process can spawn child processes, so an abandoned tab can keep spawning them in the background.

The rule is therefore that a screen whose polling is expensive must accept a `visible` prop and gate its timer on it, and every poll should additionally skip while `document.visibilityState` is not `"visible"`. Kanban already worked this way; [[src/renderer/src/screens/Workers/Workers.tsx]] did not, and its refresh re-probes every CLI path and spawns a `--version` process per hit (see [[agenthub-workers]]), so a visited-but-hidden Workers tab produced a periodic stall that looked like the whole app "stuttering every so often". [[src/renderer/src/screens/Layout/StatusBar.tsx]] is always on screen and legitimately polls, but still skips while the window is hidden or minimised.
...
[[src/renderer/src/screens/Office/OfficePixel.tsx]] polls the AgentHub activity IPC on a visible-tab timer and drops a selection whose agent left the roster, so the Office tab reflects disconnects without a manual reload. It no longer maps Hermes profiles: the office draws AgentHub agents — see [[office-pixel]].
