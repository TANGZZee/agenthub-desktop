...
### Named custom providers get their own rail entry

Brand alone cannot identify a **named** custom provider — every one carries the stored provider `custom`, so two sharing a base URL merged into one group and their identically-named models became indistinguishable rows.

[[src/renderer/src/screens/Chat/hooks/useModelConfig.ts#groupModelsByProvider]] therefore keys each group by `groupKey`, which is `label:<providerLabel>` when the model carries a named custom provider and the display brand otherwise. `provider` stays the brand (it feeds the rail logo) while `providerLabel` becomes the visible header/subtitle, so the two entries read as `Provider A · gpt-4o` and `Provider B · gpt-4o`. The model's `providerLabel` travels from [[src/main/models.ts#listModels]] through the picker; `label` grouping is display-only and never changes `provider`/`baseUrl` routing.
