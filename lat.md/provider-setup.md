...
[[src/renderer/src/components/ProviderKeysSection.tsx#ProviderModelsManager]] renders below the key field in the config modal: a key-status line, the model pills, and an add-input. It reads/writes the same `models.json` library the chat picker reads (`listModels`/`addModel`/`removeModel`, and re-syncs on `onModelLibraryChanged`), so added models immediately appear in the chat model picker. Models show as chips with a remove button, a **pencil** button, and the chip label itself — all three open a small editor for the model's shared definition (display name + context window — see [[model-context]]); because the definition is keyed by model id, editing it under one provider reflects under every provider serving that id. A chip that has a context window set also renders it inline (`128K`), so a configured value is visible without hovering. The add-input autocompletes off live discovery and strips whitespace as typed/pasted (model IDs never contain spaces, so `"hello there"` can't be saved).

#### Bulk fetch and bulk remove

A **Fetch all** pill imports every advertised model the library doesn't already have; a **Remove all** pill beside it clears the ones listed.

Discovery results previously fed only the add-input's autocomplete, so importing a provider's catalogue meant typing ids one at a time. Fetch-all counts the difference between [[src/renderer/src/hooks/useDiscoveredModels.ts#useDiscoveredModels]] results and the saved models, and persists each through the same `addModel` call the single-input path uses (`provider`/`baseUrl`/`providerLabel` identical), so routing and dedup cannot diverge between the two entry points. It reloads once after the batch and reports the count; an unreachable provider surfaces a failure note instead of a silently empty result. The pill is hidden until discovery returns at least one model, and disabled when nothing is left to add.

Remove-all is the deliberate mirror: a catalogue imported in one click must be undoable in one click, or a mistyped endpoint or an over-broad `/models` list leaves hundreds of rows behind. It iterates the models **currently listed for this provider** — the same `belongs` filter that scopes the chips — so it can never delete another provider's models, and it removes through `removeModel` so the library's change notification still fires. Both pills share the manager's `busy` flag, so the two bulk actions cannot interleave.

#### The add-input autocomplete is a real element

The add-input's suggestions render as an in-page listbox, not a native `<datalist>`.

A `<datalist>` popup is drawn by the browser: it cannot be height-limited or scrolled, so with a provider advertising a large catalogue the list ran past the window edge and the remaining entries were unreachable. The replacement is a normal element with `max-height` and `overflow-y: auto`, so every entry stays reachable. Arrow keys move a highlight and Enter commits it (falling back to the typed value when nothing is highlighted); Escape closes the list first and only collapses the input on a second press, so a stray Escape cannot discard a typed id. Suggestions are picked on `mousedown` rather than `click` so the choice wins its race with the input's blur handler.

**The popup must escape the modal.** `.models-modal` sets `overflow: hidden` to clip its frosted background to the rounded corners, which clipped this absolutely-positioned popup too — the list looked short and the lower entries could not be scrolled to. `.models-modal.provider-modal` therefore sets `overflow: visible`; that is safe because the popup is the only element that escapes, and it is bounded by its own `max-height`. This is the same failure the model-select modal already worked around, and it is why a fixed `max-height` alone was not sufficient.

The bulk pills are also deliberately **not** gated on the add-input being closed. An earlier version hid them while the field was open, so they appeared to "show up after you pick a model" — what actually happened was the field closing re-revealed them. Keeping them mounted gives both bulk actions a stable position.
...
#### Provider identity, not just the endpoint

Discovery results are cached per **provider identity**, which for a named custom provider includes its label — not only `provider|baseUrl`.

Two named custom providers may legitimately point at one gateway base URL while serving different catalogues behind different keys (that separation is the whole point of [[src/shared/url-key-map.ts#customProviderEnvKey]]). Keying the cache by endpoint alone made the second provider hit the first one's entry: [[src/main/model-discovery.ts#discoverProviderModels]] returned the cached list with `cached: true` before ever issuing a request, so the picker showed the wrong models and the provider's own key was never used.

[[src/main/model-discovery.ts#cacheKey]] therefore appends the lowercased label when the caller supplies one, and the label also participates in the `.env` lookup so `CUSTOM_PROVIDER_<NAME>_KEY` wins over the URL-derived key. The label is threaded from [[src/renderer/src/components/ProviderKeysSection.tsx#ProviderModelsManager]] (`providerLabel` → the `useDiscoveredModels` arg → the `discover-provider-models` IPC param). Native providers pass no label, so their cache behaviour — including the shared-`(provider, baseUrl)` reuse the context-window cache depends on — is unchanged. Same-label repeat calls still hit the cache ([model-context](#model-context) reads the same key shape).
